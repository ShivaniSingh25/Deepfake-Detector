...
def build_dataset(cfg, split: str):
    return FFPPDataset(
        ...
        balance=False,  # keep val/test distribution natural
    )
...
def main():
    parser = argparse.ArgumentParser(description="Threshold search for Stage 1 deepfake detector")
    parser.add_argument("--config", type=str, required=True, help="Path to train_stage1.yaml")
    parser.add_argument("--ckpt", type=str, required=True, help="Path to stage1_best checkpoint")
    parser.add_argument("--split", type=str, default="val", choices=["val", "test"], help="Which split to run on")
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--num_workers", type=int, default=4)
    parser.add_argument("--out_dir", type=str, default="results/threshold_search")
    parser.add_argument("--threshold", type=float, default=None, help="If set, only evaluate this threshold")
