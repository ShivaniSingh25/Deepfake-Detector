...
def consistency_guard(pred_label: str, text: str, confidence: str, region: str):
    ...
    # reject prompt echo / structured echo
    banned_markers = [
        "prediction:",
        "fake probability:",
        "real probability:",
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Detector + localizer + constrained LLM explanation")
    ...
    parser.add_argument("--output", type=str, default="results/explanations/explanation_result.png")
    ...
    detector = load_detector(args.ckpt, args.config, device)
    ...
    pred_label, confidence_score, fake_prob, heatmap = run_detector(
        detector, image_tensor, args.threshold
    )
    ...
    visualize_result(
        image=image,
        heatmap=heatmap,
        pred=f"{pred_label} | conf={confidence_score:.2%} | fake_prob={fake_prob:.4f}",
